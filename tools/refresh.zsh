...
print "\nnext, one step:"
print "  list page → Import… → ${all_jsonl:t}"
print ""
print "That is all. A capture puts the link in the list and the card deck and marks it read, so"
print "there is nothing to add to the worklist — the worklist is only for links you must open"
print "in a browser yourself."
if [[ -s $unresolved ]]; then
  print ""
  print "$(grep -c . "$unresolved") links resolved to nothing. Those are the only ones worth queueing:"
